...
public class ZhiZhangAIService implements AIService {
    ...
    public Point getPoint(byte[][] chessData, Point point) {
        ...
        if (this.aiConfig.isDebug()) {
            WuziqiMod.LOGGER.debug("============AI 统计[第" + this.rounds + "回合]==========");
            WuziqiMod.LOGGER.debug("搜索深度：" + this.statistics.getDepth());
            WuziqiMod.LOGGER.debug("搜索节点数：" + this.statistics.getNodes());
            WuziqiMod.LOGGER.debug("发生剪枝数：" + this.statistics.getCuts());
            WuziqiMod.LOGGER.debug("缓存总数：" + this.statistics.getCaches());
            WuziqiMod.LOGGER.debug("缓存命中数：" + this.statistics.getCacheHits());
            if (vcx > 0) {
                WuziqiMod.LOGGER.debug("算杀深度：" + this.statistics.getVcxDepth());
                WuziqiMod.LOGGER.debug("算杀命中：" + (this.statistics.getVcx() == 1 ? "VCF" : this.statistics.getVcx() == 2 ? "VCT" : "未命中"));
            }
            WuziqiMod.LOGGER.debug("最佳落子点：" + this.statistics.getPoint());
            WuziqiMod.LOGGER.debug("得分：" + this.statistics.getScore());
            double time = this.statistics.getMinimaxTime() + this.statistics.getVcxTime();
            WuziqiMod.LOGGER.debug("耗时：" + String.format("%.3f", time) + "s" + (vcx > 0 ? ", VCX(" + this.statistics.getVcxTime() + "s)" : "") + ", MINIMAX(" + this.statistics.getMinimaxTime() + "s)");
            WuziqiMod.LOGGER.debug("==================================");
        }
        ...
        for (; depth <= maxDepth; depth += 2) {
            ...
            if (point != null) {
                if (this.aiConfig.isDebug()) {
                    ...
                    //WuziqiMod.LOGGER.debug(pathOut.toString());
                }
    ...
    private int minimax(int type, int depth, int alpha, int beta) {
        ...
        if (isRoot && pointList.size() == 1) {
            ...
            // 只有一个落子点，直接返回就好
            this.bestPoint = pointList.getFirst();
        ...
        if (isRoot) {
            ...
            if (count == 1) {
                this.bestPoint = bestPointList.getFirst();
